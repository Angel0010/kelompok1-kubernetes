...
func TestListTasks_WithStatusFilter(t *testing.T) {
	...
	// Create 2 tasks, one 'done', one default
	...
	listResp := doRequest(t, srv, http.MethodGet, "/api/v1/tasks?status=done", nil)
	var listBody model.TaskListResponse
	decodeBody(t, listResp, &listBody)
	
	if len(listBody.Tasks) != 1 || listBody.Tasks[0].ID != task2.ID {
		t.Errorf("expected 1 task with status done, got %d", len(listBody.Tasks))
	}
}
...
func TestUpdateTask_TitleOnly(t *testing.T) {
	...
	decodeBody(t, upResp, &upTask)
	
	if upTask.Title != "New Title" {
		t.Errorf("title not updated: %s", upTask.Title)
	}
	if upTask.Status != "todo" {
		t.Errorf("status should not change: %s", upTask.Status)
	}
}
...
func TestStats_ConsistencyWithTaskList(t *testing.T) {
	...
	doRequest(t, srv, http.MethodPost, "/api/v1/tasks", map[string]string{"title": "Task B"})
	...
	if len(listBody.Tasks) != stats.Total {
		t.Errorf("stats total (%d) != list length (%d)", stats.Total, len(listBody.Tasks))
	}
}
